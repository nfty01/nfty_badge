...
RegisterNetEvent('ntfy_badge:open')
AddEventHandler('ntfy_badge:open', function( data, type)
	open = true
	SendNUIMessage({
		action = "open",
		array  = data,
...
RegisterNetEvent('ntfy_badge:shot')
AddEventHandler('ntfy_badge:shot', function(playerID)
...
RegisterNetEvent("pd:OpenMenu")
AddEventHandler("pd:OpenMenu", function()
openMenu()
end)
...
Citizen.CreateThread(function()
	while true do 
		...
		if IsControlJustReleased(0, Keys['F5']) and not open then
			TriggerServerEvent("kicsiafaszod")
		end
		if IsControlJustReleased(0, Keys['ESC']) and open or IsControlJustReleased(0, Keys['BACKSPACE']) and open or IsControlJustReleased(0, Keys['F5']) and open then 
			SendNUIMessage({
				action = "close"
			})
			open = false
...
function openMenu()
	ESX.UI.Menu.Open(
	  'default', GetCurrentResourceName(), 'id_card_menu', {
		  ...
		  align    = 'left',
		  elements = {
			  {label = _U('view_badge'), value = 'checkID'},
			  {label = _U('show_badge'), value = 'showID'}
		  }
	  },
	  function(data, menu)
		  ...
		  if val == 'checkID' then
			  TriggerServerEvent('ntfy_badge:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()))
		 else
			  local player, distance = ESX.Game.GetClosestPlayer()
			  
			  if distance ~= -1 and distance <= 3.0 then
				  if val == 'showID' then
				  	TriggerServerEvent('ntfy_badge:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(player))
				  end
			  else
				ESX.ShowNotification(_U('no_plyer'))
			  end
		  end
	  end,
	  function(data, menu)
